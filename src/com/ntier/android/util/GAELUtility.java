...
//usage: static public GAELUtility mGAELUtility;    
...
public class GAELUtility extends ContextWrapper {

private static GAELUtility	mGAELUtility;
private static final Logger	mLog	= LoggerFactory.getLogger( GAELUtility.class );

private static final String		ExternalStorageState	= Environment.getExternalStorageState()
								, SHARED_PREFS_FNAME = "GAEL.sharedprefs"
...
public static GAELUtility getInstance(Context context){ 
	 if (mGAELUtility == null) {
         //Always pass in the Application Context
//		 mGAELUtility = new GAELUtility(context.getApplicationContext());
		 mGAELUtility = new GAELUtility(context);
...
//GAELUtility has been initialized or an exception is thrown!
public static GAELUtility getInstance(){ 
	 if (mGAELUtility == null) {throw new GaelException(GaelErrCode.INITIALIZATION_NEEDED); }//ERRROR GAELUtility must be initialized!
    return mGAELUtility;
}

//private constructor prevents instantiation.  We only need/want one of these.
private GAELUtility(Context context) { 
	super( context );
...
public Properties getPropertiesFromAssets(String AssetFilename){
	...
	try {		
		...
		GaelException ASSETFILE_NOT_FOUND = new GaelException(GaelErrCode.ASSETFILE_NOT_FOUND)
